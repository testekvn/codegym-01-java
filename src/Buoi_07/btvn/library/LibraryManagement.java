...
//        List<Document> bookList = documentMap.get("1");
        ...
        for (Document document : documentList) {
            ...
            //if (type.equalsIgnoreCase(typeDoc)) { // 1 = 1
                ...
                if (!documentMap.containsKey(typeDoc)) {    // Kiem tra xem map co key 01 chua
                    tmp = new ArrayList<>();                // Neu chua co key la 1 thi se tao danh sach trong (danh sach nay la value cho key 1)
                } else {
                    // Lay ra danh sach book da co
                    tmp = documentMap.get(typeDoc);         // Neu map da co key thi lay ra danh sach hien tai
                }
                tmp = !documentMap.containsKey(typeDoc) ? new ArrayList<>() : documentMap.get(typeDoc);

                // Add Book vao list
                tmp.add(document);                          // Add them gia tri hien tai vao list
                // Tao danh sach book trong map vs key = 1
                documentMap.put(typeDoc, tmp);              // Cap nhat lai gia tri cua map do
            //}
        }
